...
Backend service with auth, routing, and middleware. Demonstrates per-error-variant postconditions, effect tracking, and `panics: never`.
...
fn serve(config: Config, handler: Handler) -> Result<Server, ServerError>
  ensures:
    ok => result.port == config.port
    ok => result.host == config.host
...
fn route(method: Method, pattern: String, handler: Handler) -> Route
  ensures:
...
fn handle_request(req: Request) -> Response
  ensures:
    no_matching_route(req) => result.status == 404
    handler_error => result.status == 500
    result.status == 500 => not result.body.contains_stack_trace()
    result.headers.has("X-Request-Duration")
  ...
  response.with_header("X-Request-Duration", elapsed.as_millis().to_string())
```
